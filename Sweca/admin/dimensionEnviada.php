...
<?php
    ...
    if(isset($end_insert)){
        ...
        if(trim($nombre) == ""){
            $error = true;
            echo "Error al enviar el nombre.";
        }
        if(trim($comentario) == ""){
            $error = true;
            echo "Error al enviar el comentario.";
        }
    }if(isset($end_insert) && $error == false){
        $insert = "insert into Dimensiones(id_Estudio, nombre , comentario) values('$id_estudio', '$nombre', '$comentario')";
        mysqli_query($conn, $insert) or (printf("%s", mysqli_error($conn)) and die ("Error al insertar Dimension_Nombre.")) ;
        echo "<h2>La nueva dimensión '$nombre', con un comentario <br> '$comentario' <br>ha sido insertado correctamente.</h2>";
        mysqli_close($conn);
    }
    ?>
        </div>
